...
// Pass simple objects by value, for better code generated by a call to
// non-inlined `CheckOpResult()`.
template <typename T>
struct CheckArgType {
 private:
  using Raw = std::decay_t<const T>;

 public:
  using type = std::conditional_t<std::is_trivially_copyable_v<Raw> &&
                                      std::is_copy_constructible_v<Raw> &&
                                      sizeof(Raw) <= 2 * sizeof(void*),
                                  Raw, const Raw&>;
};

template <typename T>
using CheckArgTypeT = typename CheckArgType<T>::type;

// Indicates that a check failed with the message header
// "Check failed in function: assertion (a vs. b)".
//
// Template parameters `A` and `B` should be specified explicitly.
template <typename A, typename B>
ABSL_ATTRIBUTE_COLD CheckResult CheckOpResult(const char* function,
                                              const char* assertion, A a, B b) {
...
#define RIEGELI_INTERNAL_DEFINE_CHECK_OP(name, op)                             \
  ...
  constexpr CheckResult Check##name(                                           \
    ...
    CheckResult check_result =                                                 \
        CheckOpResult<CheckArgTypeT<A>, CheckArgTypeT<B>>(function, assertion, \
                                                          a, b);               \
