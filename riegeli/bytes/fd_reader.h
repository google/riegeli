...
class FdStreamReader : public FdStreamReaderBase {
 public:
  ...
  explicit FdStreamReader(const Src& src, Options options);
  explicit FdStreamReader(Src&& src, Options options);
...
// Support CTAD.
#if __cplusplus >= 201703
FdStreamReader(int src, FdStreamReaderBase::Options options)->FdStreamReader<>;
template <typename Src>
FdStreamReader(Src&& src, FdStreamReaderBase::Options options)
    -> FdStreamReader<std::decay_t<Src>>;
template <typename... SrcArgs>
FdStreamReader(std::tuple<SrcArgs...> src_args,
               FdStreamReaderBase::Options options)
