...
class RecordWriterOptions {
 public:
  ...
  // break if they are not). A chunk of records will be processed in a way which
  ...
  //
  // Default: true.
  RecordWriterOptions& set_transpose(bool transpose) & {
  ...
  // but this is not very useful, since the reason for transpose is to make the
  // data compress better.
  RecordWriterOptions& DisableCompression() & {
    transpose_ = false;
  ...
  RecordWriterOptions& EnableBrotliCompression(int level = 9) & {
  ...
  RecordWriterOptions& EnableZstdCompression(int level = 9) & {
  ...
  // allows to read pieces of the file independently with finer granularity, and
  // reduces memory usage of both writer and reader.
  //
  // Default: 1 << 20
  RecordWriterOptions& set_desired_chunk_size(size_t size) & {
  ...
  // several fields of the given wire type to be compressed together, relatively
  // to the desired chunk size, on the scale between 0.0f (compress each field
  // separately) to 1.0f (put all fields of the same wire type in the same
  // bucket).
  //
  ...
  //
  // Default: 1.0f
  RecordWriterOptions& set_desired_bucket_fraction(float fraction) & {
  ...
  // Larger parallelism can increase throughput, up to a point where it no
  // longer matters; smaller parallelism reduces memory usage.
  //
  // If parallelism > 0, chunks are written to the byte Writer in background and
  // reporting writing errors is delayed.
  //
  // Default: 0
  RecordWriterOptions& set_parallelism(int parallelism) & {
